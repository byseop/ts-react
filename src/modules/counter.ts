...
export const increaseby = createStandardAction(INCREASE_BY)<number>(); // payload 타입을 Generics로 넣어준다.
...
const actions = { increase, decrease, increaseby }; // 모든 액션생성함수들을 actions 객체에 넣는다.
type CounterAction = ActionType<typeof actions>; // ActionType 을 사용하여 모든 액션객체들의 타입을 준비해준다.
...
const counter = createReducer<CounterState, CounterAction>(initialState, {
  [INCREASE]: state => ({ count: state.count + 1 }), // 액션을 참조할 필요 없으면 파라미터로 state만 가져와도 됨
